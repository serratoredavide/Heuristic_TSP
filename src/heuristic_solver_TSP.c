    ...
    while (swap != 0) {
        ...
        for (int i = 1; i < num_nodes - 1; i++) {
            for (int k = i + 1; k < num_nodes; k++) {
                c1 = dist(tour[i - 1], tour[i], inst);
                c2 = dist(tour[k], tour[(k + 1) % num_nodes], inst);
                c3 = dist(tour[i - 1], tour[k], inst);
                c4 = dist(tour[i], tour[(k + 1) % num_nodes], inst);
                delta = (c3 + c4) - (c1 + c2);
                if (delta < best_delta) {
